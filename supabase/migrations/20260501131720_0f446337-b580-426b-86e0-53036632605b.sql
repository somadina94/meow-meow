-- Exclude busy users from "online" dashboard listing
-- Busy = currently in audio/video/private-group call (status_text='busy')
...
BEGIN
  ...
  INNER JOIN user_status us
    ...
    AND COALESCE(us.status_text, 'online') <> 'busy'  -- hide users in active calls/groups
  LEFT JOIN LATERAL (
