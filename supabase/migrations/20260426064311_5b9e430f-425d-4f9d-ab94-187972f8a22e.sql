...
-- Drop both overloads cleanly to remove the ambiguity
DROP FUNCTION IF EXISTS public.get_analytics_summary(timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_analytics_summary(timestamptz, timestamptz, text);
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'men_recharges', COALESCE((
      SELECT sum(credit) FROM ledger_transactions
      WHERE transaction_type = 'recharge' AND credit > 0
        AND created_at >= p_start_date AND created_at <= p_end_date
    ), 0),
    'men_spent', COALESCE((
      SELECT sum(debit) FROM ledger_transactions
      WHERE transaction_type IN (
        'chat_charge','video_call_charge','audio_call_charge',
        'group_call_charge','tip_charge'
      )
      AND debit > 0
      AND created_at >= p_start_date AND created_at <= p_end_date
    ), 0),
    'women_earnings', COALESCE((
      SELECT sum(credit) FROM ledger_transactions
      WHERE transaction_type IN (
        'earning','chat_earning','video_call_earning',
        'audio_call_earning','group_call_earning','tip_earning'
      )
      AND credit > 0
      AND created_at >= p_start_date AND created_at <= p_end_date
    ), 0),
    'women_withdrawals', COALESCE((
      ...
      WHERE status IN ('completed','processing','approved')
        AND created_at >= p_start_date AND created_at <= p_end_date
    ), 0),
