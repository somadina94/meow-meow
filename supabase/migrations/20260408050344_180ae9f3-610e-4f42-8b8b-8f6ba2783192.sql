...
-- Fix process_group_tip: remove is_indian gate, add platform_ledger entries
...
BEGIN
  ...
  -- Platform ledger for man's tip debit
  ...
  IF v_women_share > 0 THEN
    ...
    -- Platform ledger for woman's tip earning
...
-- Fix historical wallet_transactions with null transaction_type
UPDATE public.wallet_transactions
SET transaction_type = 'tip_charge'
WHERE transaction_type IS NULL
  AND type = 'debit'
  AND description LIKE 'Group tip:%';

UPDATE public.wallet_transactions
SET transaction_type = 'gift_charge'
WHERE transaction_type IS NULL
  AND type = 'debit'
  AND description LIKE 'Gift:%';
