-- Count active men reliably (DISTINCT + male_profiles fallback when profiles.gender is stale/null).
...
AS $$
  SELECT count(DISTINCT gcp.user_id)::int
    ...
    LEFT JOIN public.profiles p ON p.user_id = gcp.user_id
   WHERE gcp.session_id = p_session_id
     ...
     AND (
       ...
       OR EXISTS (SELECT 1 FROM public.male_profiles mp WHERE mp.user_id = gcp.user_id)
     );
$$;
...
BEGIN
  ...
  SELECT lower(trim(gender)) INTO v_gender
   ...
   LIMIT 1;
  IF v_gender IS DISTINCT FROM 'male'
     AND NOT EXISTS (SELECT 1 FROM public.male_profiles mp WHERE mp.user_id = p_man_id) THEN
  ...
  v_active_men := public.group_chat_active_men_count(p_session_id, v_session.host_id);
