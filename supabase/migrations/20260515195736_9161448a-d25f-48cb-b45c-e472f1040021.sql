...
BEGIN
  ...
  LOOP
    ...
    -- Lock wallet for ANY positive balance (KYC state irrelevant for the reset)
    IF v_bal > 0 THEN
    ...
    -- Reset wallet for ANY positive balance (regardless of KYC) so women dashboard shows ₹0
    IF v_inserted > 0 AND v_bal > 0 AND v_wallet.id IS NOT NULL THEN
      ...
      IF v_kyc_state = 'approved' THEN
        ...
        VALUES (v_rec.user_id, 'Payout queued',
      ...
      ELSIF v_kyc_state = 'kyc_pending' THEN
        INSERT INTO public.notifications (user_id, title, message, type, action_url)
        VALUES (v_rec.user_id, 'Complete your KYC to receive payout',
          '₹' || ROUND(v_bal, 2)::text || ' has been moved from your wallet to the payout queue. Submit your bank KYC so admin can release it.',
          'kyc_required', '/kyc');
        v_count_kyc_pending := v_count_kyc_pending + 1;
      ELSIF v_kyc_state = 'kyc_rejected' THEN
        INSERT INTO public.notifications (user_id, title, message, type, action_url)
        VALUES (v_rec.user_id, 'KYC rejected — payout on hold',
          '₹' || ROUND(v_bal, 2)::text || ' has been moved to the payout queue but is on hold. Update your bank KYC to release it.',
          'kyc_required', '/kyc');
...
BEGIN
  ...
  LOOP
    ...
    IF v_bal > 0 AND v_wallet.id IS NOT NULL THEN
