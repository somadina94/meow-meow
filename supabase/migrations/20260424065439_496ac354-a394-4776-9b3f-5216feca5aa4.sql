...
BEGIN
  ...
  -- Multi-host: prefer the specific host the sender joined
  ...
  -- Use 'gift_debit' (allowed by platform_ledger entry_type check)
  v_ref_key := 'tip:' || v_txn_id::text;
  PERFORM public.safe_ledger_insert(
    p_sender_id, p_group_id, 'gift_debit',
  ...
  IF v_women_share > 0 THEN
    ...
    -- Use 'gift_credit' (allowed by platform_ledger entry_type check)
    v_ref_key := 'tip_earn:' || v_txn_id::text;
    PERFORM public.safe_ledger_insert(
      v_host_id, p_group_id, 'gift_credit',
