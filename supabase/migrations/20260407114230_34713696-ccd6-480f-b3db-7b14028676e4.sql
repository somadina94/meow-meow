...
-- First, delete the stale monthly_statements that have wrong data
DELETE FROM public.monthly_statements 
WHERE user_id = '0b933372-7f04-4397-9aae-0e8be4730702';

-- Fix get_my_statement_summary: remove floor on back-calculated opening and cross-validate monthly_statements
...
BEGIN
  ...
  -- Always compute opening from ledger data
  ...
  -- Use ledger-computed value; only use monthly_statements if ledger has no data and ms exists
  ...
  IF v_is_current_month THEN
    ...
    -- Back-calculate opening so equation holds: opening = closing - credit + debit
    -- Do NOT floor this - it must balance the equation exactly
...
-- Fix get_my_statement_detail: cross-validate and use correct opening for running_balance
...
BEGIN
  ...
  FROM (
    SELECT credit as cr, debit as dr FROM public.ledger_transactions lt2
