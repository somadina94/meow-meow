...
BEGIN
  ...
  v_billable_seconds := FLOOR(v_elapsed_seconds)::integer;
  ...
  IF v_billable_seconds < 1 THEN
    RETURN jsonb_build_object('success', true, 'duplicate_skipped', true, 'too_soon', true, 'charged', 0, 'earned', 0, 'elapsed_seconds', v_elapsed_seconds, 'idempotency_key', v_idem_key);
  END IF;
  ...
  v_billable_minutes := v_billable_seconds::numeric / 60.0;
  ...
  IF NOT v_is_super THEN
    ...
    IF v_charge > v_man_balance THEN
      v_billable_seconds := GREATEST(0, FLOOR((v_man_balance / NULLIF(v_man_rate, 0)) * 60.0)::integer);
      v_billable_minutes := v_billable_seconds::numeric / 60.0;
      v_charge := ROUND(v_billable_minutes * v_man_rate, 2);
      v_earn := ROUND(v_billable_minutes * v_woman_rate, 2);
      v_should_end := true;
  ...
  IF v_billable_seconds < 1 OR (NOT v_is_super AND v_charge <= 0) THEN
    UPDATE public.active_chat_sessions
    SET status = 'ended', ended_at = v_billed_at, end_reason = 'insufficient_funds', updated_at = v_billed_at
    WHERE id = p_session_id;
    RETURN jsonb_build_object('success', false, 'error', 'Insufficient balance', 'session_ended', true, 'charged', 0, 'earned', 0);
  ...
  v_metadata := jsonb_build_object(
    ...
    'billed_seconds', v_billable_seconds,
    'billed_minutes', ROUND(v_billable_minutes, 6),
  ...
  IF NOT v_is_super THEN
    ...
    INSERT INTO public.wallet_transactions (
      ...
      'Chat billing: ' || v_billable_seconds || ' sec @ ₹' || to_char(v_man_rate, 'FM999999990.00') || '/min; woman rate ₹' || to_char(v_woman_rate, 'FM999999990.00') || '/min',
      v_idem_key, p_session_id, 'chat', v_man_balance, v_idem_key, 'completed', v_billable_seconds, v_man_rate, v_metadata
    );
  END IF;
  ...
  IF v_earn > 0 AND v_woman_indian THEN
    ...
    IF v_woman_wallet_id IS NOT NULL THEN
      ...
      INSERT INTO public.wallet_transactions (
        ...
        'Chat earning: ' || v_billable_seconds || ' sec @ ₹' || to_char(v_woman_rate, 'FM999999990.00') || '/min; man rate ₹' || to_char(v_man_rate, 'FM999999990.00') || '/min',
        v_idem_key_w, p_session_id, 'chat', v_woman_balance, v_idem_key_w, 'completed', v_billable_seconds, v_woman_rate, v_metadata || jsonb_build_object('idempotency_key', v_idem_key_w)
      );
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'duration_seconds', v_billable_seconds,
    'elapsed_seconds', ROUND(v_elapsed_seconds, 3),
    'billable_minutes', ROUND(v_billable_minutes, 6),
    'man_rate_per_minute', v_man_rate,
    'woman_rate_per_minute', v_woman_rate,
    'woman_is_indian', v_woman_indian,
    'idempotency_key', v_idem_key,
