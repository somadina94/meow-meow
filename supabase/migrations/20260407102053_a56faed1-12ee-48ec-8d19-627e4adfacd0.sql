...
-- 1) Add columns to wallet_transactions
ALTER TABLE public.wallet_transactions
  ADD COLUMN IF NOT EXISTS duration_seconds integer DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS rate_per_minute numeric(12,2) DEFAULT NULL;

-- 2) Update ledger_bill_session to store duration + rate
...
-- 3) Update ledger_bill_group_call to store duration + rate
...
-- 4) Update get_my_statement_detail to read duration + rate from wallet_transactions
