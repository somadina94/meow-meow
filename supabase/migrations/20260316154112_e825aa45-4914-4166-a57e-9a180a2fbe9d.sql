...
-- Returns summary counts + daily chart data in one call
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'women_earnings', COALESCE((SELECT sum(amount) FROM women_earnings WHERE created_at >= p_start_date AND created_at <= p_end_date), 0),
  ...
  -- === Gender distribution (always full) ===
  SELECT jsonb_build_object(
  ...
  -- === Daily chart data (server-side aggregation) ===
