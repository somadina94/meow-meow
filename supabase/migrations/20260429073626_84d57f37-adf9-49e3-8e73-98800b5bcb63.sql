...
BEGIN
  ...
  -- Authorization: caller must be the man, the woman host (group calls), service_role, or admin
  IF auth.uid() IS NOT NULL
     AND auth.role() <> 'service_role'
     AND auth.uid() IS DISTINCT FROM v_man_id
     AND auth.uid() IS DISTINCT FROM v_woman_id
  ...
  IF v_earn > 0 THEN
    ...
    INSERT INTO public.wallet_transactions (
      ...
      v_label || ' earning — ' || p_minutes || ' min @ ₹' || v_woman_rate || '/min',
  ...
  RETURN jsonb_build_object(
    ...
    'charged', CASE WHEN v_is_admin THEN 0 ELSE v_charge END,
    'earned', v_earn,
    'man_rate', v_man_rate,
    'woman_rate', v_woman_rate,
    'minutes', p_minutes,
    'super_user_skip', v_is_admin,
