-- Women's online list was showing a man's oldest user_languages row
-- (ORDER BY created_at ASC), so leftover Telugu stayed after switching to Hindi.
-- Prefer profiles.primary_language (what LanguageSelector writes), then the
-- newest user_languages row.
