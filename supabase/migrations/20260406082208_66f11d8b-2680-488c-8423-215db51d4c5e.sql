...
-- ============================================================
-- User-facing statement RPCs (scoped to auth.uid())
-- Uses ledger_transactions for men, women_earnings + withdrawal_requests for women
-- Temp tables for running balance calculation
-- ============================================================

-- 1. get_my_statement_summary — monthly summary for current user
CREATE OR REPLACE FUNCTION public.get_my_statement_summary(
  p_year  integer,
  p_month integer
)
RETURNS jsonb
...
BEGIN
  ...
  -- Carry-forward: get prior month closing
  ...
  IF v_opening_balance IS NULL THEN
    -- Fallback: compute from ledger if no prior statement exists
...
-- 2. get_my_statement_detail — transaction rows with running balance
CREATE OR REPLACE FUNCTION public.get_my_statement_detail(
  p_year  integer,
  p_month integer
)
RETURNS TABLE (
...
BEGIN
  ...
  -- Opening balance (same logic as summary)
  IF p_month = 1 THEN v_prev_year := p_year - 1; v_prev_month := 12;
  ELSE v_prev_year := p_year; v_prev_month := p_month - 1;
  END IF;
  ...
  IF v_opening IS NULL THEN
    IF v_gender = 'male' THEN
      SELECT COALESCE(SUM(credit) - SUM(debit), 0) INTO v_opening
      FROM public.ledger_transactions
      WHERE user_id = v_user_id AND created_at < v_period_start;
    ELSE
      SELECT COALESCE(
  ...
  -- Use temp table for running balance computation
  ...
  ELSE
    ...
    -- Women: approved withdrawals as debits
    INSERT INTO _stmt_rows (txn_date, transaction_id, session_id, txn_type, description, duration_seconds, rate_per_minute, debit, credit)
    SELECT
      wr.processed_at,
      wr.id::text,
  ...
  -- Return with running balance using window function
  RETURN QUERY
  SELECT
    sr.txn_date,
    sr.transaction_id,
...
GRANT EXECUTE ON FUNCTION public.get_my_statement_summary(integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_my_statement_detail(integer, integer) TO authenticated;
