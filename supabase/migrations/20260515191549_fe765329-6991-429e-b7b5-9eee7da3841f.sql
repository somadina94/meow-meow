...
BEGIN
  ...
  LOOP
    ...
    -- Lock wallet row & re-read live balance (avoid race with billing)
    ...
    IF NOT v_kyc_ok THEN
      INSERT INTO public.women_payout_snapshots (
        ...
        v_wallet.balance, 0, v_wallet.balance, v_wallet.balance, 'failed',
    ...
    -- ALWAYS zero the wallet (KYC-failed too) — this is what admin expects.
    -- Failed-KYC payouts remain owed to the user via the snapshot row; admin
    -- can later re-credit after KYC is approved.
    IF v_inserted > 0 THEN
      ...
      INSERT INTO public.wallet_transactions (
        ...
        v_wallet.balance, 0,
        CASE WHEN v_kyc_ok
          THEN 'Payout to bank account — admin generated on ' || TO_CHAR(v_ist_now, 'DD Mon YYYY HH24:MI IST')
          ELSE 'Payout snapshot (KYC pending) — wallet reset on ' || TO_CHAR(v_ist_now, 'DD Mon YYYY HH24:MI IST')
        END,
