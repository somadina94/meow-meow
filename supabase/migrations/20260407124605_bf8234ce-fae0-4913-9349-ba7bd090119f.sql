...
-- Fix get_my_statement_detail: remove blanket skip, use per-row dedup
...
RETURNS TABLE (
  ...
  rate_per_minute numeric, debit numeric, credit numeric,
  running_balance numeric, start_time timestamptz, end_time timestamptz
)
...
BEGIN
  ...
  -- 2) Wallet transactions (deduplicated against ledger by idempotency_key OR by session+amount+time)
  INSERT INTO _stmt_rows (txn_date, transaction_id, session_id, txn_type, description, duration_seconds, rate_per_minute, debit, credit, start_time, end_time)
  ...
  WHERE wt.user_id = v_user_id AND wt.created_at >= v_period_start AND wt.created_at < v_period_end AND wt.status = 'completed'
    -- Skip if matched by idempotency_key in ledger
    AND NOT EXISTS (
    ...
    -- Skip if matched by session_id + amount + close time in ledger (for entries without idempotency_key)
    AND NOT EXISTS (
      SELECT 1 FROM public.ledger_transactions lt
  ...
  -- 3) Platform ledger (deduplicated against ledger + wallet_transactions)
  INSERT INTO _stmt_rows (txn_date, transaction_id, session_id, txn_type, description, duration_seconds, rate_per_minute, debit, credit, start_time, end_time)
  ...
  -- 4) Women earnings — ALWAYS included for female users, with per-row dedup (NO blanket skip)
  ...
  ELSE
    -- For past months, compute opening from all pre-period transactions
...
BEGIN
  ...
  FROM (
    ...
    UNION ALL
    -- Wallet transactions (deduped vs ledger)
    SELECT
      CASE WHEN wt.type = 'debit' THEN wt.amount ELSE 0 END,
      CASE WHEN wt.type = 'credit' THEN wt.amount ELSE 0 END
    FROM public.wallet_transactions wt
    ...
    UNION ALL
    -- Platform ledger (deduped vs ledger + wallet)
    ...
    UNION ALL
    -- Women earnings (per-row dedup, NO blanket skip)
  ...
  ELSE
    -- Compute opening from all pre-period transactions
