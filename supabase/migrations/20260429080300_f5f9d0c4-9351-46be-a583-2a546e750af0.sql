...
CREATE OR REPLACE FUNCTION public.get_ledger_statement(p_user_id uuid, p_from_date text DEFAULT NULL::text, p_to_date text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, session_id text, session_type text, transaction_type text, debit numeric, credit numeric, description text, reference_id text, counterparty_id text, running_balance numeric, created_at timestamp with time zone, duration_seconds integer, rate_per_minute numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN QUERY
  WITH unified AS (
    SELECT wt.id, wt.session_id, wt.session_type, wt.type, wt.transaction_type, wt.amount,
    ...
    UNION ALL
    SELECT wa.id, wa.session_id, wa.session_type, wa.type, wa.transaction_type, wa.amount,
    ...
    SELECT u.id,
           ...
           u.session_type::text AS session_type,
  ...
  SELECT
    f.id, f.session_id, f.session_type, f.transaction_type, f.debit, f.credit,
...
GRANT EXECUTE ON FUNCTION public.get_ledger_statement(uuid, text, text) TO authenticated, service_role, anon;
