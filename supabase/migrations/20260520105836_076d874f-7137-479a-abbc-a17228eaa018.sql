...
AS $function$
  SELECT
    u AS user_id,
    GREATEST(COALESCE((
      SELECT SUM(CASE WHEN wt.type = 'credit' THEN wt.amount
                      WHEN wt.type = 'debit'  THEN -wt.amount
                      ELSE 0 END)
      FROM (
        SELECT type, amount, status FROM public.wallet_transactions WHERE user_id = u
        UNION ALL
        SELECT type, amount, status FROM public.wallet_transactions_archive WHERE user_id = u
      ) wt
...
BEGIN
  ...
  -- Canonical balance from wallet_transactions (credits minus debits, completed only)
  LEFT JOIN LATERAL (
    ...
    FROM (
      SELECT type, amount, status FROM public.wallet_transactions WHERE user_id = p.user_id
      UNION ALL
      SELECT type, amount, status FROM public.wallet_transactions_archive WHERE user_id = p.user_id
    ) wt
    WHERE wt.status = 'completed'
  ) wb ON true
  LEFT JOIN LATERAL (
